    ...
    private suspend fun downloadSystemPluginFromGitHub(plugin: SystemPluginInfo): Boolean {
        return withContext(Dispatchers.IO) {
            try {
                ...
                //
                // Skipped for downloadOnly plugins, matching backfillSidecarIfMissing
                // (only called from the non-downloadOnly branch of
                // ensureSystemPluginsInstalled). The microkernel runtime is the only
                // such plugin and PluginLoaderDelegateImpl refuses to install it as
                // one, so it never reaches the loader's signature check. Signing it
                // bought nothing and cost a getDownloadUrl — a booked
                // `plugin_downloads` row — plus a warn on every launch, for a JAR
                // that most likely has no store row to be signed against.
                if (!plugin.downloadOnly) {
